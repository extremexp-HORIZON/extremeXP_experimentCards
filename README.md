...
2. **Set Up Environment Variables**:
   ...
   DAL_SYNC_ENABLED=(true/false)
   DAL_SYNC_INTERVAL_MINUTES=60
   DAL_BASE_URL=https://api.dal.extremexp-icom.intracom-telecom.com/api
   ```
   Enabling the sync(DAL_SYNC_ENABLED=true) causes the application to poll the DAL `/experiments` feed every `DAL_SYNC_INTERVAL_MINUTES` to grab the latest experiment/workflow/metric payloads.
